...
std::string ConstructHttpMessage()
{
    std::string crlf = "\r\n";
...
TEST(HttpRespParserTest, HttpRespParser)
{
    std::string content = ConstructHttpMessage();
    HttpRespParser parser;
    for (size_t i = 0; i < content.size(); i++) {
        std::string msg(1, '0');
        msg[0] = content[i];
        auto httpResp = parser.Parse(msg);
        if (i != content.size() - 1) {
            EXPECT_EQ(httpResp.has_value(), false);
        } else {
            EXPECT_EQ(httpResp.has_value(), true);
        }
    }
}
